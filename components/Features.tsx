      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div id="reserve-card" className="order-1 md:order-2 group relative bg-[#FFFEFA]/90 backdrop-blur-md border border-stone-200 rounded-[40px] p-10 overflow-hidden hover:border-stone-300 transition-all duration-500 min-h-[640px] flex flex-col shadow-xl">
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'intro' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             ...
             <div className="flex-1 flex items-center justify-center w-full">
                <div className="animate-float-slow">
                    <img 
                        ...
                        className={`
                            w-full max-w-[220px] md:max-w-[280px] max-h-[220px] object-contain drop-shadow-lg cursor-pointer
             ...
             <div className="mt-8 w-full space-y-3">
                <button 
                    ...
                    className="w-full py-4 bg-[#F7F5F0]/80 hover:bg-[#F0EBE0] backdrop-blur-md text-stone-900 border border-[#E8E6df] text-sm font-bold rounded-xl transition-all flex items-center justify-center gap-2 shadow-sm active:scale-[0.98]"
                >
                    이미 예매하셨나요?
                </button>
                <button 
                    onClick={handleStartTicket}
                    className="w-full py-4 bg-stone-900 text-[#F7F5F0] text-sm font-bold rounded-xl hover:bg-stone-800 transition-all flex items-center justify-center gap-2 shadow-lg active:scale-[0.98]"
                >
                    티켓 예매 시작하기
                </button>
             </div>
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'countSelection' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none translate-x-10'}`}>
             <div className="flex-1 flex flex-col gap-5 flex-1 justify-center">
                <div className="space-y-4">
                    <label className="text-sm font-bold text-stone-500 flex items-center justify-center gap-2"><Ticket className="w-5 h-5" /> 예매 매수</label>
                    <div className="flex items-center justify-center gap-10 py-4">
                        <button onClick={() => handleInputChange('count', Math.max(1, formData.count - 1))} className="text-4xl font-light text-stone-800 hover:opacity-50 transition-opacity w-12 h-12 flex items-center justify-center">-</button>
                        <span className="font-black text-stone-900 text-5xl">{formData.count}매</span>
                        <button onClick={() => handleInputChange('count', Math.min(10, formData.count + 1))} className="text-4xl font-light text-stone-800 hover:opacity-50 transition-opacity w-12 h-12 flex items-center justify-center">+</button>
                    </div>
                    <div className="text-center">
                         <p className="text-lg font-bold text-stone-500 mt-4">
             ...
             <div className="mt-8 w-full">
                <button 
                    ...
                    className={`w-full py-4 text-sm font-bold rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg bg-stone-900 text-[#F7F5F0] hover:bg-stone-800 active:scale-[0.98]`}
                >
                   {isSubmitting ? (
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'confirm' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             <div className="flex-1 flex flex-col justify-center overflow-y-auto custom-scrollbar">
                <div className="text-center space-y-2 mb-8">
                    <h4 className="text-xl font-bold text-stone-900">틀린 곳이 있나요?</h4>
                    <p className="text-sm text-red-500 font-medium leading-relaxed">예매 확정 후에는 정보를 바꾸기 어려워요.</p>
                </div>
                <div className="bg-stone-50 backdrop-blur-md border border-stone-200 rounded-2xl p-6 space-y-4">
                    <div className="flex justify-between border-b border-stone-200 pb-2">
                        <span className="text-xs text-stone-500">성함 / 뒷번호</span>
                        <span className="text-sm font-bold text-stone-900">{formData.name} / {formData.phone}</span>
                    </div>
                    <div className="flex justify-between border-b border-stone-200 pb-2">
                        <span className="text-xs text-stone-500">지인 (카테고리)</span>
                        <span className="text-sm font-bold text-stone-900">{formData.referral}</span>
                    </div>
                    <div className="flex justify-between border-b border-stone-200 pb-2">
                        <span className="text-xs text-stone-500">예매 수량</span>
                        <span className="text-sm font-bold text-stone-900">{formData.count}매</span>
                    </div>
                    <div className="flex justify-between pt-2">
                        <span className="text-xs text-stone-500">최종 결제 금액</span>
                        <span className="text-lg font-bold text-red-500">{ (formData.count * TICKET_PRICE).toLocaleString() }원</span>
                    </div>
                </div>
             </div>
             <div className="mt-8 w-full flex gap-3">
                <button 
                  ...
                  className="flex-1 py-4 bg-stone-100 border border-stone-200 text-sm font-bold text-stone-900 rounded-xl active:scale-[0.98] transition-all disabled:opacity-50 hover:bg-stone-200"
                >
                  수정
                </button>
                <button 
                  ...
                  disabled={isSubmitting} 
                  className={`flex-1 py-4 bg-stone-900 text-[#F7F5F0] text-sm font-bold rounded-xl flex items-center justify-center active:scale-[0.98] transition-all relative overflow-hidden shadow-lg hover:bg-stone-800 ${!isSubmitting ? 'gap-2' : ''}`}
                >
                    {isSubmitting ? (
                      <Loader2 className="w-5 h-5 animate-spin" />
                    ) : (
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'success' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             <div className="flex-1 flex flex-col justify-center overflow-y-auto custom-scrollbar">
                <div className="flex flex-col items-center py-4">
                    <h2 className="text-2xl font-bold text-stone-900 mb-1">{(formData.count * TICKET_PRICE).toLocaleString()}원</h2>
                    <p className="text-sm text-stone-500 font-medium">1매 10,000원 × {formData.count}매</p>
                </div>
                <div className="w-full border-t border-stone-200 my-2"></div>
                <div className="flex flex-col items-center text-center py-5">
                    <p className="text-sm font-bold text-stone-900 mb-2">보내는 분 이름을</p>
                    <p className="text-xl font-bold text-red-500 mb-2">'{formData.name}{formData.phone}'</p>
                    <p className="text-sm font-bold text-stone-900 mb-2">로 바꿔주세요.</p>
                    <p className="text-[11px] text-stone-400 font-medium">* 위 문구와 다를 시 입금 확인이 지연될 수 있습니다.</p>
                </div>
                <div className="w-full border-t border-stone-200 my-2"></div>
                <div className="flex items-center justify-between py-4 px-2">
                    <div className="space-y-1">
                        <span className="bg-[#FAE100] text-[#3B1E1E] text-[10px] font-bold px-1.5 py-0.5 rounded inline-block">국민은행</span>
                        <p className="text-xl font-bold font-mono text-stone-900 tracking-tighter mt-1">21490204079748</p>
                        <p className="text-xs text-stone-500 font-medium">예금주: 안성준</p>
                    </div>
                    <button 
                        onClick={() => { 
                        ...
                        className="w-12 h-12 rounded-full bg-stone-100 backdrop-blur-md border border-stone-200 shadow-lg flex items-center justify-center hover:bg-stone-200 transition-all active:scale-95 group/copy"
                    >
                        {isCopied ? <Check className="w-5 h-5 text-green-500" /> : <Copy className="w-5 h-5 text-stone-400 group-hover/copy:text-stone-600" />}
                    </button>
                </div>
             </div>
             <div className="mt-4 w-full">
                {showCopyWarning && (
                     <p className="text-center text-xs text-red-500 font-bold mb-2 animate-pulse">
                        계좌번호 복사버튼을 먼저 눌러주세요
                     </p>
                )}
                <button onClick={() => {
                ...
                }} className="w-full py-4 bg-stone-900 text-[#F7F5F0] text-sm font-bold rounded-xl hover:bg-stone-800 transition-all shadow-lg active:scale-[0.98]">
                    입금 완료했어요
                </button>
             </div>
          </div>
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'finalGuide' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             <div className="flex-1 flex flex-col justify-center items-center text-center overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-6">
                    <h3 className="text-xl font-bold text-stone-900">계좌 입금이 완료되었나요?</h3>
             ...
             <div className="mt-8 w-full">
                <button onClick={() => setReserveStage('intro')} className="w-full py-4 bg-stone-900 text-[#F7F5F0] text-sm font-bold rounded-xl hover:bg-stone-800 transition-all shadow-lg active:scale-[0.98]">
                    내 티켓 발급하러 가기
                </button>
             </div>
          </div>
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'duplicateError' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             <div className="flex-1 flex flex-col items-center justify-center text-center overflow-y-auto custom-scrollbar">
                ...
                <div className="w-full mb-8 space-y-2 shrink-0">
                     <p className="text-xs text-stone-400 font-medium mb-1">입력하신 정보</p>
                     <p className="text-lg font-bold text-stone-900">{formData.name} / {formData.phone} / {formData.count}매</p>
                </div>

                {/* Divider */}
                <div className="w-full border-t border-stone-200 mb-8 shrink-0"></div>

                {/* Error Message */}
                <div className="w-full shrink-0">
                    <p className="text-sm text-red-500 font-bold leading-relaxed whitespace-pre-wrap">
                        이미 입력된 정보입니다.<br/>
                        수정이 필요하다면<br/>
             ...
             <div className="flex-1 flex flex-col justify-center gap-6">
                <div className="space-y-4">
                    <div className="space-y-1.5">
                        <label className="text-xs font-semibold text-stone-500">예매자 성함</label>
                        <input type="text" placeholder="홍길동" value={lookupData.name} onChange={(e) => setLookupData(prev => ({...prev, name: e.target.value}))} className={`w-full bg-stone-50 border rounded-xl px-4 py-4 text-sm text-stone-900 focus:outline-none placeholder-stone-400 ${lookupData.name && !isLookupNameValid ? 'border-red-400' : 'border-stone-200'}`} />
                        {lookupData.name && !isLookupNameValid && (
                             <p className="text-xs text-red-500 font-medium pl-1 mt-1">숫자 및 특수기호는 불가합니다</p>
                        )}
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-xs font-semibold text-stone-500">연락처 뒷번호 4자리</label>
                        <input type="tel" maxLength={4} placeholder="1234" value={lookupData.phone} onChange={(e) => setLookupData(prev => ({...prev, phone: e.target.value.replace(/[^0-9]/g, '')}))} className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-4 text-sm text-stone-900 tracking-widest focus:outline-none placeholder-stone-400" />
                    </div>
                </div>
                <div className="bg-stone-50 rounded-xl p-4 flex gap-3">
                    <AlertCircle className="w-5 h-5 text-stone-400 shrink-0" />
                    <p className="text-[11px] text-stone-500 leading-relaxed">입금 확인은 수동으로 진행되며, 입금 후 최대 24시간이 소요될 수 있습니다.</p>
                </div>
             </div>
             <div className="mt-8 w-full">
                <button onClick={handleLookup} className="w-full py-4 bg-stone-900 text-[#F7F5F0] text-sm font-bold rounded-xl shadow-lg flex items-center justify-center gap-2 hover:bg-stone-800 active:scale-[0.98]">
                    정보 확인하기 <ChevronRight className="w-4 h-4" />
                </button>
             </div>
          </div>
          ...
          <div className={`absolute inset-0 top-32 px-10 pb-10 flex flex-col transition-all duration-500 ${reserveStage === 'checkLoading' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             <div className="flex-1 flex flex-col items-center justify-center gap-10">
                <div className="flex flex-col items-center gap-2">
                     ...
                     <h3 className="text-2xl font-black text-stone-900">{lookupData.name} {lookupData.phone}</h3>
                </div>
                ...
                <div className="bg-stone-100 px-6 py-3 rounded-full">
                    <p className="text-xs font-bold text-stone-500 animate-pulse">데이터를 검색 중입니다</p>
                </div>
             </div>
          </div>
                ...
                {lookupResult?.status === 'success' ? (
                    ...
                    <div className="flex flex-col items-center gap-6 py-10 text-center">
                        <div className="w-48 h-48 flex items-center justify-center">
                            <img 
                                src="https://i.postimg.cc/0QbwnWY3/Gemini-Generated-Image-94x63x94x63x94x6-removebg-preview.png" 
                                alt="Not Found" 
                                className="w-full h-full object-contain" 
                            />
                        </div>
                        <div className="space-y-1">
                            <h4 className="text-xl font-bold text-stone-900">정보 없음</h4>
                            <p className="text-sm text-stone-500">입력 정보를 다시 확인해주세요.</p>
                        </div>
                    </div>
                ) : (
                    <div className="flex flex-col items-center gap-6 py-10 text-center">
                        <div className="w-48 h-48 flex items-center justify-center">
                            <img 
                                src="https://i.postimg.cc/D0GbkSYg/Gemini-Generated-Image-fd2u5jfd2u5jfd2u-removebg-preview.png" 
                                alt="Pending Verification" 
                                className="w-full h-full object-contain" 
                            />
                        </div>
                        <div className="space-y-1">
                            <h4 className="text-xl font-bold text-stone-900">입금 확인 중입니다</h4>
                            <p className="text-sm text-stone-500">입금 내역을 확인하고 있습니다. <br/>조금만 더 기다려 주세요!</p>
                        </div>
                    </div>
                )}
             </div>
        ...
        <div id="location-card" className="order-2 md:order-1 group relative bg-[#FFFEFA]/90 backdrop-blur-md border border-stone-200 rounded-[40px] p-10 overflow-hidden hover:border-stone-300 transition-all duration-500 min-h-[640px] flex flex-col shadow-xl">
           ...
           <div className={`absolute inset-0 top-0 p-10 flex flex-col transition-all duration-500 overflow-y-auto custom-scrollbar ${infoStage === 'info' ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none -translate-x-10'}`}>
                ...
                <div className="relative z-10 mb-4 flex justify-between items-start min-h-[48px] shrink-0">
                    <div className="flex-1 pr-4">
                        <h3 className="text-2xl font-bold text-stone-900 mb-1">공연 정보</h3>
                        <p className="text-sm font-medium text-[#1A1A1A]/60">밴드 공연 | 180분</p>
                    </div>
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center w-full min-h-0">
                    ...
                    <div className="w-full shrink-0 mb-4">
                        {/* Changed Header to match SET.3 style */}
                        <h4 className="text-stone-900 font-black text-lg block mb-2">관람 정보</h4>
                        {/* Updated content wrapper to match Band Members style */}
                        <div className="space-y-2 border-t border-stone-200 pt-3 text-sm text-[#1A1A1A]/60 font-medium leading-relaxed">
                            <div className="flex justify-between items-center">
                                <span className="shrink-0">제목</span>
                                <span className="text-right">赤玉春 VOL.2</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="shrink-0">장소</span>
                                <span className="text-right">펄스 라이브홀</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="shrink-0">날짜</span>
                                <span className="text-right">02.14 SAT</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="shrink-0">시간</span>
                                <span className="text-right">16:00 - 19:00</span>
                            </div>
                        </div>
                    </div>
                </div>
           ...
           <div className={`absolute inset-0 top-0 p-10 flex flex-col h-full w-full transition-all duration-500 ease-in-out ${infoStage === 'map' ? 'opacity-100 translate-x-0 z-10' : (infoStage === 'info' ? 'opacity-0 translate-x-10 pointer-events-none' : 'opacity-0 -translate-x-10 pointer-events-none')}`}>
              <div className="flex items-center justify-between mb-4 shrink-0">
                  <h3 className="text-xl font-bold text-stone-900">찾아오시는 길</h3>
                  <button 
              ...
              <div className="relative z-10 shrink-0">
                <p className="text-sm text-stone-700 leading-relaxed w-full font-medium whitespace-pre-wrap">
                  {`도로명 주소 : 서울특별시 서초구 주흥길 12 B1\n지번 주소 : 서울특별시 서초구 반포동 741 B1\n강남 교보타워 사거리 인근,\n9호선 신논현역 1번출구 도보 350m`}
                </p>
              </div>
              <div className="flex-1 w-full mt-6 mb-6 rounded-3xl overflow-hidden relative border border-stone-200 bg-stone-50 shadow-inner min-h-[200px]">
          ...
          <div className={`absolute inset-0 top-0 p-10 flex flex-col h-full w-full transition-all duration-500 ease-in-out ${infoStage === 'parking' ? 'opacity-100 translate-x-0 z-10' : 'opacity-0 translate-x-10 pointer-events-none'}`}>
              <div className="flex items-center justify-between mb-6 shrink-0">
                  <h3 className="text-xl font-bold text-stone-900">공영 주차장 안내</h3>
                  <button 
                      onClick={() => setInfoStage('map')}
                      className="w-10 h-10 rounded-full bg-black/5 flex items-center justify-center hover:bg-black/10 transition-colors shrink-0"
                  >
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4">
                   ...
                   <div className="bg-stone-50 border border-stone-200 rounded-2xl p-5">
                       <h4 className="font-bold text-stone-900 text-lg mb-1">1. 언구비 공영 주차장</h4>
                       <p className="text-xs text-red-500 font-bold mb-3">공연장과 20M 거리</p>
                       <div className="space-y-1 text-sm text-stone-700">
                          <p>강남대로83길 55</p>
                          <div className="flex gap-3 font-medium text-stone-500 mt-2 text-xs">
                   ...
                   {/* Parking Item 2 */}
                   <div className="bg-stone-50 border border-stone-200 rounded-2xl p-5">
                       <h4 className="font-bold text-stone-900 text-lg mb-1">2. 방음언덕형 공영 주차장</h4>
                       <p className="text-xs text-red-500 font-bold mb-3">공연장과 150M 거리</p>
                       <div className="space-y-1 text-sm text-stone-700">
                          <p>주흥3길 34</p>
                          <div className="flex gap-3 font-medium text-stone-500 mt-2 text-xs">
                              <span>60분 2,000원</span>
              ...
              <div className="mt-6 w-full shrink-0">
                   <button onClick={() => setInfoStage('info')} className="w-full py-4 bg-stone-900 text-[#F7F5F0] text-sm font-bold rounded-xl hover:bg-stone-800 transition-all shadow-lg active:scale-[0.98]">
                      돌아가기
                   </button>
              </div>
          </div>
        ...
        <div id="members-card" className="order-4 md:order-4 group relative bg-[#FFFEFA]/90 backdrop-blur-md border border-stone-200 rounded-[40px] p-10 overflow-hidden hover:border-stone-300 transition-all duration-500 min-h-[500px] flex flex-col shadow-xl">
          <div className="relative z-10 mb-8">
            {/* CHANGED: text-xl -> text-2xl */}
            <h3 className="text-2xl font-bold text-stone-900 mb-6">밴드 구성원</h3>
            <div className="space-y-2 text-sm text-[#1A1A1A]/60 font-medium leading-relaxed">
          ...
          <div className="flex-1 flex items-center justify-center select-none relative w-full">
              <div className="relative w-full h-full max-h-[280px] flex items-center justify-center">
                  <img 
                      src="https://i.postimg.cc/Y96w5kzz/Gemini-Generated-Image-ke8a1vke8a1vke8a-removebg-preview.png" 
                      alt="Band Illustration" 
                      className="relative z-10 w-full h-full object-contain drop-shadow-sm opacity-90 animate-float-slow"
                  />
                  {/* Shadow */}
                  <div className="absolute -bottom-2 w-[60%] h-6 bg-black/10 blur-xl rounded-[100%] z-0"></div>
              </div>
          </div>
        </div>

        {/* Playlist Card - UPDATED COLOR & ANIMATION */}
        <div id="playlist-card" className="order-5 md:order-5 group relative bg-[#FFFEFA]/90 backdrop-blur-md border border-stone-200 rounded-[40px] p-10 overflow-hidden hover:border-stone-300 transition-all duration-500 min-h-[500px] flex flex-col shadow-xl">
          <div className="relative z-10 w-full mb-10">
            {/* CHANGED: text-xl -> text-2xl */}
            <h3 className="text-2xl font-bold text-stone-900 mb-8">플레이 리스트</h3>
            <div className="flex flex-col gap-8 text-sm text-[#1A1A1A]/60 font-medium leading-relaxed">
              <div>
                <span className="text-stone-900 font-black text-lg block mb-2">SET.1</span>
                <p>등대</p>
                <p>FIND ME!</p>
              </div>
              <div>
                <span className="text-stone-900 font-black text-lg block mb-2">SET.2</span>
                <p>Close to you</p>
              ...
              <div>
                <span className="text-stone-900 font-black text-lg block mb-2">SET.3</span>
                <p>시퍼런 봄</p>
                <p>꽃에 망령</p>
                <p>계속 웃을 순 없어!</p>
                <p>숙명</p>
              ...
              <div>
                <span className="text-stone-900 font-black text-lg block mb-2">SET.4</span>
                <p>불꽃놀이</p>
                <p>ㅈㅣㅂ</p>
                <p>Drowning</p>
                <p>항해</p>
          ...
          <div className="flex-1 flex items-center justify-center select-none relative w-full">
              <img 
                  src="https://i.postimg.cc/g2ZtSsDZ/Gemini-Generated-Image-afnqh0afnqh0afnq-removebg-preview.png" 
